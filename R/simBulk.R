...
#' test of the model. Then, \code{num.bulk.samples} bulk samples proportions are
#' built, each one composed by \code{n.cells} (100 by default). The proportions
#' of training and test bulk samples are set by \code{train.freq.bulk} (2/3 for
#' training and 1/3 for test by default). Finally, in order to avoid biases due
#' to the composition of bulk samples, proportions for the mixtures (bulk
#' samples) of cell types (\eqn{w_1,...,w_k}, where \eqn{k} is the number of
#' cell types available in single-cell profiles) are randomly generated by using
...
#'   bulk samples) that will be generated taking into account training and test
#'   data. We recommend seting this value according to the number of single-cell
#'   profiles available in \code{\linkS4class{DigitalDLSorter}} object (we must
#'   be more specific).
...
#' Simulate training and test simulated bulk RNA-seq samples
#'
