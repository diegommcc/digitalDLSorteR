...
#'
#' Simulate training and test pseudo-bulk RNA-Seq profiles using cell
#' composition matrices built by \code{\link{generateBulkCellMatrix}} function.
#' Samples are generated under the assumption that expression level of gene
#' \eqn{i} in bulk sample \eqn{j} is given by the sum of expression levels of
#' cell types \eqn{X_{ijk}} that compose them weighted by the proportions of
#' these cell types \eqn{k} in each sample. In practice, as described in Torroja
#' and Sanchez-Cabo, 2019, these profiles are generated by the summation of a
#' number of cells from different cell types determined by proportions from a
#' known cell composition matrix. The number of simulated pseudo-`bulk RNA-Seq
#' samples and the number of cells that compose each sample are determined by
