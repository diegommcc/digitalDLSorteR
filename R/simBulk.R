...
#'   cell type.}
#' @param num.bulk.samples Number of bulk samples proportions (and, therefore,
...
#' @param train.freq.cells Proportion of cells used for training set (2/3 by
#'   default).
#' @param train.freq.bulk Proportion of bulk samples used for training set (2/3
#'   by default).
#' @param proportions.train Vector of five integers that determine the
...
#' types \eqn{k} in each sample. In practice, as described by Torroja et al.,
#' 2019, these profiles are generated by the summation of a number of cells from
#' different cell types determined by proportions from a known cell composition
#' matrix. The number of simulated bulk samples and the number of cells that
#' compose each sample are determined by \code{\link{generateBulkCellMatrix}}
#' (see Documentation) \strong{Note:} this step can be avoid by using
#' \code{on.the.fly} argument in \code{\link{trainDigitalDLSorterModel}}
...
#' @param block.size Number of bulk expression profiles that will be simulated
#'   in each iteration during the process. Larger numbers resulting in more
#'   memory usage but lesser execution times. Set according to available
