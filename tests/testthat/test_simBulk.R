...
test_that(
  "Wrong object: no single-cell profiles or cell type matrix", 
  {
    ...
    expect_error(
      ...
      regexp = "There are no single-cell profiles in DigitalDLSorter object"
    )
    # no prob matrix
    expect_error(
      simBulkProfiles(
...
test_that(
  "Check expected behaviour", 
  {
    ...
    DDLSLiT <- simBulkProfiles(
      object = DDLSLi,
      type.data = "train"
    )
    expect_null(bulk.simul(DDLSLiT, type.data = "test"))
...
if (requireNamespace("DelayedArray", quietly = TRUE) || 
  ...
  test_that(
    paste("Using HDF5 files as back-end simBulkProfiles: the following", 
