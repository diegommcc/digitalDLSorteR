...
```r
devtools::load_all("~/Documents/Rpackages/digitalDLSorteR")
```

```
## Warning in env_get_list(ns, names(active_bindings)[!active_bindings]): internal error -3 in R_decompress1
```

```
## Error in env_get_list(ns, names(active_bindings)[!active_bindings]): lazy-load database '/home/dmananesc/R/x86_64-pc-linux-gnu-library/4.3/digitalDLSorteR/R/digitalDLSorteR.rdb' is corrupt
```
